...
| 배포 ref | `main` |
| 현재 source 및 origin/main | `2f1a11175ea2fe6537da5fb1bfac80d0e8f426ad` |
...
기존 endpoint의 기대 출력은 `10`, `16`, `82`다. v1.1.0 새 endpoint의 기대
출력은 `92`, `16`, `82`다.
...
공개 프런트엔드에서 확인한 기존 서버 상세의 lifecycle 동작은 시작·중지·삭제이며,
소스 ref를 수정해 재빌드하는 별도 동작은 확인되지 않았다. 이 때문에 아래
**새 서버로 검증 후 전환**을 기본 절차로 둔다. 계정 화면에 별도 `재빌드` 또는
`소스 갱신` 기능이 보이면 그 기능을 사용해도 되지만, 새 endpoint에서 4.4의
검증을 반드시 다시 수행한다.
...
3. PlayMCP in KC에 같은 카카오 계정으로 로그인해 **새 MCP 서버 등록 → Git 소스 빌드**를
   연다.
...
4.4의 검증 결과를 첨부해 해당 안내의 운영 채널에 변경을 요청해야 한다.
...
5. 새 endpoint를 만든 뒤 4.4를 통과시키고 본선 endpoint 전환을 요청한다.
...
- [ ] v1.1.0 커밋을 GitHub `main`에 push한다.
